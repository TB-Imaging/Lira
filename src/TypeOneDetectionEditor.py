...
class TypeOneDetectionEditor(object):
    ...
    def mouse_click(self, event):
        # Do the same thing for left/right click, start a selection rect. Our rectangle selections can only be made
        # up of small rectangles of size step_h*step_w, so that we lock on to areas in these step sizes to allow
        # easier rectangle selection.
        ...
        outline_rect_x1, outline_rect_y1, outline_rect_x2, outline_rect_y2 = get_outline_rectangle_coordinates(
            self.selection_x1, self.selection_y1, self.selection_x1, self.selection_y1, self.step_h, self.step_w)
        self.canvas.create_rectangle(outline_rect_x1, outline_rect_y1, outline_rect_x2, outline_rect_y2, fill='',
                                     outline="darkRed", width=2, tags="selection")

    def mouse_move(self, event):
        # Do the same thing for left/right move, move the selection rect. Our rectangle selections can only be made
        # up of small rectangles of size step_h*step_w, so that we lock on to areas in these step sizes to allow
        # easier rectangle selection.
    ...
    def mouse_left_release(self, event):
        # Remove all detections from the area in this rectangle selection, then add detections of size rect_hxrect_w
        # in intervals step_hxstep_w in the area in this rectangle selection. Then update the detections. Our
        # rectangle selections can only be made up of small rectangles of size step_hxstep_w, so that we lock on to
        # areas in these step sizes to allow easier rectangle selection.
        ...
        for i, detection in reversed(list(enumerate(self.detections))):
            if detection_in_rect(detection, [outline_rect_x1, outline_rect_y1, outline_rect_x2, outline_rect_y2],
    ...
    def mouse_right_release(self, event):
        # Remove all detections from the area in this rectangle selection, Then update the detections. Our rectangle
        # selections can only be made up of small rectangles of size step_h*step_w, so that we lock on to areas in
    ...
    def update_detections(self):
        ...
        for detection in self.detections:
            self.canvas.create_rectangle(detection[0], detection[1], detection[2], detection[3], fill='', outline="red",
                                         width=2, tags="detection")

        # Convert our local detections copy back into a np array, scale it back up, cast it to int, and update the
        # stored detections.
        self.dataset.type_one_detections.after_editing[self.dataset.progress["type_ones_image"]] = (
                    np.array(self.detections) / self.editor_resize_factor).astype(int)
    ...
    def reload_img_and_detections(self):
        ...
        self.detections = list(self.dataset.type_one_detections.after_editing[self.dataset.progress[
            "type_ones_image"]] * self.editor_resize_factor)  # Make list so we can append
